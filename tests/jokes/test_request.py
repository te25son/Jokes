...
from jokes.options import Category, Flag, Language, Type
from jokes.pipelines.get_pipeline import deserialize, make_request
from jokes.utils.params import GetEndpointParams


class TestRequest:
    @pytest.mark.parametrize(
        "type, category, flags, lang",
        [
    ...
    def test_complete_response(
        self, type: Type, category: Category, flags: list[Flag], lang: Language
    ):
        data = self._create_option_data(category, type, flags, lang)
        response = unsafe_perform_io(
            make_request(data).bind_result(deserialize).unwrap()
        )
